...
const ScheduleRow = memo(function (props) {
    ...
    return (
        <li>
            <div className={classnames('icon', { 'icon-red': isDepartStaion || isArriveStation })}>
                {
                    isDepartStaion ? '出' : isArriveStation ? '到' : leftPad(index, 0, 2)
                }
            </div>
            <div className={classnames('row', { 'grey': beforeDepartStation || afterArrivrStation })}>
                <span className={classnames('station', { 'red': isStartStation || isEndStation })}>{station}</span>
                <span className={classnames('arrive', { 'red': isArriveStation })}>{isStartStation ? '始发站' : arriveTime}</span>
                <span className={classnames('deptime', { 'red': isEndStation })}>{isEndStation ? '终到站' : departTime}</span>
